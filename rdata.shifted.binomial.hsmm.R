rdata.shifted.binomial.hsmm<-function(NUM, par_dwell, pro, f0, pc, f1)
{

## USAGE
 # rdata.binomial.hsmm(NUM, Pi, prob, f0, ...)
 # par_dwell=list(n1, Pi)

## ARGUEMENTS
 # NUM: number of hypotheses
 # par_dwell: the parameters of the shifted binomial distribution
 #
...
## DETAILS
 ...
 # where the dwell time distribution for state 0 is a shifted binomial distribution and
...
## generating the underlying states
 # Without loss of generality, assume that theta[1]=0.
	...
	{
           	theta<-c(theta, rep(0, rbinom(1, n1, Pi)+1))
